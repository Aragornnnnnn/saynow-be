...
# Excalidraw 시나리오 마인드맵 체크리스트

- [x] Excalidraw 보드에 접속해 현재 내용을 확인한다.
- [x] 기존 보드 내용을 유지할 빈 공간을 찾는다.
- [x] 각 시나리오를 카테고리별로 분류한다.
- [x] 분류 결과를 마인드맵 형태로 빈 공간에 추가한다.
- [x] 추가된 마인드맵이 보드에 반영됐는지 확인한다.
