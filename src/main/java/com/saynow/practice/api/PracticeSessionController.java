...
public class PracticeSessionController {
    ...
    public ResponseEntity<ApiResponse<SessionStartResponse>> startSession(
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(practiceSessionService.startSession(principal.memberId(), request)));
    }

    @GetMapping("/{sessionId}")
    @Operation(summary = "세션 상태 조회", description = "현재 세션 상태와 진행 턴 이력을 조회합니다.")
    public ApiResponse<SessionStatusResponse> getSession(
            @AuthenticationPrincipal AuthMemberPrincipal principal,
            @PathVariable String sessionId
    ) {
        return ApiResponse.success(practiceSessionService.getSession(principal.memberId(), sessionId));
    }

    @PutMapping("/{sessionId}/micReady")
    @Operation(summary = "마이크 준비 지연 기록", description = "세션별 마이크 준비까지 걸린 시간을 기록합니다.")
    public ApiResponse<MicReadyResponse> recordMicReady(
            @AuthenticationPrincipal AuthMemberPrincipal principal,
            @PathVariable String sessionId,
            @Valid @RequestBody MicReadyRequest request
    ) {
        return ApiResponse.success(practiceSessionService.recordMicReady(principal.memberId(), sessionId, request));
    }
    ...
    public ApiResponse<TurnSubmitResponse> submitTurn(
        ...
        return ApiResponse.success(practiceSessionService.submitTurn(principal.memberId(), sessionId, toSubmittedAudio(audio), parseSubmitTurnRequest(request)));
    }

    @PostMapping("/{sessionId}/exit")
    @Operation(summary = "세션 중도 종료", description = "진행 중인 세션을 사용자 종료 상태로 마감합니다.")
    public ApiResponse<ExitSessionResponse> exitSession(
            @AuthenticationPrincipal AuthMemberPrincipal principal,
            @PathVariable String sessionId
    ) {
        return ApiResponse.success(practiceSessionService.exitSession(principal.memberId(), sessionId));
    }
