...
class OpenApiIntegrationTest extends IntegrationTestSupport {
    ...
    void separatesSuccessAndErrorResponsesPerApi() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/scenarios'].get.responses.200.content['application/json'].examples.SUCCESS.value.data.categories[0].scenarios[0].firstQuestionPreview.aiQuestion").value("Hey, you must be my roommate! I'm charlie. Okay, tell me everything — what are you studying, what are you into?"))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.submittedTurn.sequence").value(4))
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.submittedTurn.innerThought").value("못 먹는 음식을 부드럽게 말해줘서 저녁 메뉴를 맞추기 쉽겠다."))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.nextTurn.sequence").value(5))
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.nextTurn.aiQuestion").value("Got it. I'll avoid fish, and we can share dinner tonight."))
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.nextTurn.translatedQuestion").value("알겠어. 생선은 피해서 오늘 저녁 같이 먹자."))
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.progress.currentSequence").value(5))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/feedback'].post.responses.200.content['application/json'].examples.SUCCESS.value.data.turnFeedbacks[0].originalQuestion").value("Hey, you must be my roommate! I'm charlie. Okay, tell me everything — what are you studying, what are you into?"))
