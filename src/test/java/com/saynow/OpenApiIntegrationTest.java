...
class OpenApiIntegrationTest extends IntegrationTestSupport {
    ...
    void separatesSuccessAndErrorResponsesPerApi() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/scenarios'].get.responses.200.content['application/json'].examples.SUCCESS.value.data.categories[0].scenarios[0].firstQuestionPreview.aiQuestion").value("If you could travel anywhere for free right now, where would you go? And what draws you to that place?"))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.submittedTurn.innerThought").value("마지막 답변이 분명해서 자연스럽게 마무리하면 좋겠다."))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.nextTurn.aiQuestion").value("Thanks for sharing. I hope you get to experience that someday."))
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/utterances'].post.responses.200.content['application/json'].examples.COMPLETED.value.data.nextTurn.translatedQuestion").value("이야기해줘서 고마워. 언젠가 꼭 경험해보면 좋겠다."))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/sessions/{sessionId}/feedback'].post.responses.200.content['application/json'].examples.SUCCESS.value.data.turnFeedbacks[0].originalQuestion").value("If you could travel anywhere for free right now, where would you go? And what draws you to that place?"))
